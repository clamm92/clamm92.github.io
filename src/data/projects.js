...
export const projectsData = [
  {
    ...
    "solution": "Our goal was to build 1 accessible platform combining daily risk check-ins, symptom history, and 6 education modules with quizzes to support post-discharge monitoring for non-clinical users. As a team, we defined the feature roadmap, designed navigation and question flow, and iterated with sponsor feedback to balance clinical relevance with simplicity. We validated assumptions through rapid prototyping and usability testing. I designed a multi-tier risk engine using vitals scoring, pattern detection, and clinical modifiers to provide real-time triage guidance with emergency escalation; the risk calculator is currently patent pending. I also built an adaptive check-in flow with context-driven branching to reduce input burden, shipped 5 authenticated API routes for onboarding, check-ins, and reminders, engineered schema-safe API pipelines to reduce invalid writes, set up Supabase Auth and Postgres, and converted Figma designs into production UI. Our team also built a comprehensive automated test suite covering risk logic, API routes, and UI state transitions.",
